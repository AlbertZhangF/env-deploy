...
Deploy a code project's runtime environment on Linux and leave behind reproducible deployment artifacts. Prefer the bundled automation scripts for detection, planning, execution, logging, report generation, and `setup.sh` generation.
...
2. **Run automated detection**
   - For local projects, run:
     ```bash
     python3 env_deploy/scripts/detect_project.py /path/to/project --pretty
     ```
   - For full automation planning, run:
     ```bash
     python3 env_deploy/scripts/env_deploy.py --project /path/to/project
     ```
...
python3 env_deploy/scripts/env_deploy.py --project /path/to/project --apply

# Include package-manager system package installation
python3 env_deploy/scripts/env_deploy.py --project /path/to/project --apply --install-system-packages

# Resolve ambiguity explicitly
python3 env_deploy/scripts/env_deploy.py --project /path/to/project --component-type python --apply
python3 env_deploy/scripts/env_deploy.py --project /path/to/project --component-type java --java-tool maven --apply
```
