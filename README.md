...
```bash
npx skills add AlbertZhangF/env-deploy
```
...
```bash
npx skills add AlbertZhangF/env-deploy --skill '*' --agent '*' -g -y
```
...
```bash
npx skills add AlbertZhangF/env-deploy --skill env-deploy -g -y
npx skills add AlbertZhangF/env-deploy --skill env-deploy-for-codex -a codex -g -y
npx skills add AlbertZhangF/env-deploy --skill env-deploy-for-cc -a claude-code -g -y
npx skills add AlbertZhangF/env-deploy --skill env-deploy-for-trae -a trae -g -y
```
...
```bash
npx skills add AlbertZhangF/env-deploy --list
```

The repository uses the standard `skills/<skill-name>/` layout so `npx skills add` can discover each variant directly.

本仓库使用标准 `skills/<skill-name>/` 布局，便于 `npx skills add` 直接发现各平台版本。
