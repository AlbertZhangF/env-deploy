...
if [ -n "${TRAE_SKILL_ROOT:-}" ]; then
  ...
  echo "Cannot find env_deploy_for_trae skill directory" >&2
  exit 1
fi
```

For workspace-local Trae Skills, copy this folder to `.trae/skills/env_deploy_for_trae/`. This repository does not create `.trae/` automatically.
