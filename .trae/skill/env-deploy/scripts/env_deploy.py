#!/usr/bin/env python3
"""Automated Linux environment deployment helper for env_deploy_for_trae.
...
def write_setup(ctx: DeployContext) -> None:
    lines = [
        "#!/usr/bin/env bash",
        "# Generated by env_deploy_for_trae. Review before running on another host.",
        "set -euo pipefail",
        "",
...
def resolve_source(source: str | None, clone_dir: Path | None, apply: bool) -> Path:
    ...
    if re.match(r"^(git@|https?://)", source):
        target_root = (clone_dir or (Path.cwd() / "env_deploy_for_trae_sources")).expanduser().resolve()
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Automate env_deploy_for_trae discovery, execution, logging, and artifact generation.")
...
def print_human_summary(summary: dict[str, object]) -> None:
    print("env_deploy_for_trae summary")
